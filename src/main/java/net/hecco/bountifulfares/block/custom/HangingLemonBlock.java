...
public class HangingLemonBlock extends HangingFruitBlock {
    private static final VoxelShape[] SHAPES = new VoxelShape[]{Block.box(7, 13, 7, 9, 16, 9),
            ...
            Shapes.join(Block.box(6, 10, 6, 10, 15, 10), Block.box(7, 9, 7, 9, 16, 9), BooleanOp.OR),
            Shapes.join(Block.box(5.5, 8, 5.5, 10.5, 14, 10.5), Block.box(7, 7, 7, 9, 16, 9), BooleanOp.OR)};
    ...
    public HangingLemonBlock(Properties settings) {
        super(settings);
    }
    ...
    public boolean canSurvive(BlockState state, LevelReader world, BlockPos pos) {
        return Block.canSupportCenter(world, pos.above(), Direction.DOWN) && !world.isWaterAt(pos)
                || world.getBlockState(pos.above()).is(BFBlocks.LEMON_LEAVES.get()) && !world.isWaterAt(pos)
                || world.getBlockState(pos.above()).is(BFBlocks.FLOWERING_LEMON_LEAVES.get()) && !world.isWaterAt(pos);
    }
