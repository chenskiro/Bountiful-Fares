...
public class BFGLMProvider extends GlobalLootModifierProvider {
    public BFGLMProvider(PackOutput gen, CompletableFuture<HolderLookup.Provider> registries, String modid) {
        super(gen, registries,modid);
    }

    @Override
    protected void start() {
        add("add_grass_seed", new AddItemModifier(Stream.of(Blocks.SHORT_GRASS, Blocks.TALL_GRASS, Blocks.FERN, Blocks.LARGE_FERN)
                .map(grass -> LootTableIdCondition.builder(grass.getLootTable().location())
        ...
        for (var seed : List.of(BFItems.LAPISBERRY_SEEDS, BFItems.HOARY_SEEDS)) {
            LootItemCondition lootItemCondition = LootTableIdCondition.builder(BuiltInLootTables.SNIFFER_DIGGING.location())
        ...
        LootItemCondition lootItemCondition_guardian = LootTableIdCondition.builder(EntityType.GUARDIAN.getDefaultLootTable().location())
        ...
        LootItemCondition lootItemCondition_elder_guardian = LootTableIdCondition.builder(EntityType.ELDER_GUARDIAN.getDefaultLootTable().location())
                .build();
